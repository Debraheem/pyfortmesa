...
These scripts are optional MESA-backed checks. They require a MESA-enabled
`pyfortmesa` build and a valid `MESA_DIR`.

Normal no-MESA unit checks live under `tests/unit/`. Timing and integration
...
`./test` runs the no-MESA checks and stores command logs under
